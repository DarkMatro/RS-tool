...
from modules.classificators import model_metrics
from modules.spec_functions import random_rgb, random_line_style, curve_pen_brush_by_style, set_roi_size_pos, \
    calculate_vips
...
class CommandImportFiles(QUndoCommand):
    ...
    async def redo(self) -> None:
        self.RS.beforeTime = datetime.now() - (datetime.now() - self.RS.beforeTime)
        ...
        await self.RS.update_plot_item(self.RS.ImportedArray.items())
    ...
    async def _stop(self) -> None:
        ...
        time_end = self.RS.beforeTime
        if not self.RS.beforeTime:
            time_end = datetime.now()
        seconds = round((datetime.now() - time_end).total_seconds())
        self.RS.beforeTime = None
...
class CommandDeleteInputSpectrum(QUndoCommand):
    ...
    def prepare(self) -> None:
        for i in self.selected_indexes:
            ...
            if self.rs.BeforeDespike and filename in self.rs.BeforeDespike:
                self.BeforeDespikeArray[filename] = self.rs.BeforeDespike[filename]
            if self.rs.ConvertedDict and filename in self.rs.ConvertedDict:
                self.ConvertedDict_old[filename] = self.rs.ConvertedDict[filename]
            if self.rs.CuttedFirstDict and filename in self.rs.CuttedFirstDict:
                self.CuttedFirstDict_old[filename] = self.rs.CuttedFirstDict[filename]
            if self.rs.NormalizedDict and filename in self.rs.NormalizedDict:
                self.NormalizedDict_old[filename] = self.rs.NormalizedDict[filename]
            if self.rs.SmoothedDict and filename in self.rs.SmoothedDict:
                self.SmoothedDict_old[filename] = self.rs.SmoothedDict[filename]
            if self.rs.baseline_dict and filename in self.rs.baseline_dict:
                self.BaselineDict_old[filename] = self.rs.baseline_dict[filename]
            if self.rs.baseline_corrected_dict and filename in self.rs.baseline_corrected_dict:
                self.BaselineCorrectedDict_old[filename] = self.rs.baseline_corrected_dict[filename]
            if self.rs.report_result and filename in self.rs.report_result:
                self.report_result_old[filename] = self.rs.report_result[filename]
            if self.rs.sigma3 and filename in self.rs.sigma3:
                self.sigma3_old[filename] = self.rs.sigma3[filename]
    ...
    def _del_row(self, key: str) -> None:
        ...
        if key in self.rs.BeforeDespike:
            del self.rs.BeforeDespike[key]
        if key in self.rs.ConvertedDict:
            del self.rs.ConvertedDict[key]
        if key in self.rs.NormalizedDict:
            del self.rs.NormalizedDict[key]
        if key in self.rs.SmoothedDict:
            del self.rs.SmoothedDict[key]
        if key in self.rs.CuttedFirstDict:
            del self.rs.CuttedFirstDict[key]
        if key in self.rs.baseline_dict:
            del self.rs.baseline_dict[key]
        if key in self.rs.baseline_corrected_dict:
            del self.rs.baseline_corrected_dict[key]
        if key in self.rs.report_result:
            del self.rs.report_result[key]
        if key in self.rs.sigma3:
            del self.rs.sigma3[key]
    ...
    def _add_row(self, key: str) -> None:
        ...
        if key in self.BeforeDespikeArray:
            self.rs.BeforeDespike[key] = self.BeforeDespikeArray[key]
        if key in self.ConvertedDict_old:
            self.rs.ConvertedDict[key] = self.ConvertedDict_old[key]
        if key in self.CuttedFirstDict_old:
            self.rs.CuttedFirstDict[key] = self.CuttedFirstDict_old[key]
        if key in self.NormalizedDict_old:
            self.rs.NormalizedDict[key] = self.NormalizedDict_old[key]
        if key in self.SmoothedDict_old:
            self.rs.SmoothedDict[key] = self.SmoothedDict_old[key]
        if key in self.BaselineDict_old:
            self.rs.baseline_dict[key] = self.BaselineDict_old[key]
        if key in self.BaselineCorrectedDict_old:
            self.rs.baseline_corrected_dict[key] = self.BaselineCorrectedDict_old[key]
        if key in self.report_result_old:
            self.rs.report_result[key] = self.report_result_old[key]
        if key in self.sigma3_old:
            self.rs.sigma3[key] = self.sigma3_old[key]
...
class CommandUpdateInterpolated(QUndoCommand):
    ...
    @asyncSlot()
    async def redo(self) -> None:
        if self.rs.beforeTime is None:
            self.rs.beforeTime = datetime.now()
        self.rs.beforeTime = datetime.now() + (self.rs.beforeTime - datetime.now())
        self.rs.ui.statusBar.showMessage('Redo...' + self.text())
        self.rs.disable_buttons(True)
...
class CommandUpdateDespike(QUndoCommand):
    ...
    def _redo(self, i: tuple[str, np.ndarray, list[float]]) -> None:
        ...
        if name not in self.rs.BeforeDespike:
            self.rs.BeforeDespike[name] = self.ImportedArray_before[name]
    ...
    def _undo_dict(self, i: tuple[str, np.ndarray]) -> None:
        ...
        n_array = i[1]
        self.rs.ImportedArray[name] = n_array
        if name in self.rs.BeforeDespike:
            del self.rs.BeforeDespike[name]
...
class CommandConvert(QUndoCommand):
    ...
    def __init__(self, rs, converted_list: list[tuple[str, np.ndarray, float, float]],
        ...
        self.setText(description)
        self.ConvertedDict = rs.ConvertedDict
        self.ConvertedDictBefore = dict()
        self.ConvertedDictBefore = rs.ConvertedDict.copy()
        self.input_table = rs.ui.input_table
        self.change_list = []
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
    ...
    async def redo(self) -> None:
        ...
        await self.rs.update_plot_item(self.ConvertedDict.items(), 1)
...
class CommandCutFirst(QUndoCommand):
    ...
    def __init__(self, rs, cut_list: list[tuple[str, np.ndarray]], description: str) -> None:
        ...
        self.setText(description)
        self.CuttedFirstDict = rs.CuttedFirstDict
        self.CuttedFirstDictBefore = rs.CuttedFirstDict.copy()
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
        self.loop = rs.loop
    ...
    async def redo(self) -> None:
        ...
        await self.rs.update_plot_item(self.CuttedFirstDict.items(), 2)
        create_task(self._stop())
...
class CommandNormalize(QUndoCommand):
    ...
    def __init__(self, rs, normalized_list: list[tuple[str, np.ndarray]],
        ...
        self.NormalizedDictBefore = rs.NormalizedDict.copy()
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
        self.loop = rs.loop
    ...
    async def redo(self) -> None:
        ...
        self.rs.ui.statusBar.showMessage('Redo...' + self.text())
        self.rs.disable_buttons(True)
        self.rs.NormalizedDict.clear()
        for key, array in self.normalized_list:
            self.rs.NormalizedDict[key] = array
        await self.rs.update_plot_item(self.rs.NormalizedDict.items(), 3)
    ...
    async def undo(self) -> None:
        ...
        self.rs.disable_buttons(True)
        self.rs.NormalizedDict.clear()
        self.rs.NormalizedDict = self.NormalizedDictBefore.copy()
        info('Undo normalization. The len of NormalizedDict is %s', len(self.rs.NormalizedDict))
        await self.rs.update_plot_item(self.NormalizedDictBefore.items(), 3)
...
class CommandSmooth(QUndoCommand):
    ...
    def __init__(self, rs, smoothed_list: list[tuple[str, np.ndarray]],
        ...
        self.SmoothedDictBefore = rs.SmoothedDict.copy()
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
        self.loop = rs.loop
    ...
    async def redo(self) -> None:
        ...
        self.rs.ui.statusBar.showMessage('Redo...' + self.text())
        self.rs.disable_buttons(True)
        self.rs.SmoothedDict.clear()
        for key, array in self.smoothed_list:
            self.rs.SmoothedDict[key] = array
        await self.rs.update_plot_item(self.rs.SmoothedDict.items(), 4)
    ...
    async def undo(self) -> None:
        ...
        self.rs.disable_buttons(True)
        self.rs.SmoothedDict.clear()
        self.rs.SmoothedDict = self.SmoothedDictBefore.copy()
        info('Undo smoothing. The len of SmoothedDict is %s', len(self.rs.SmoothedDict))
        await self.rs.update_plot_item(self.SmoothedDictBefore.items(), 4)
...
class CommandBaselineCorrection(QUndoCommand):
    ...
    def __init__(self, rs, baseline_corrected_list: list[tuple[str, np.ndarray, np.ndarray]],
        ...
        self.method_new = self.generate_title_text()
        self.baseline_corrected_dict_before = rs.baseline_corrected_dict.copy()
        self.baseline_dict_before = rs.baseline_dict.copy()
    ...
    async def redo(self) -> None:
        ...
        self.rs.ui.statusBar.showMessage('Redo...' + self.text())
        self.rs.disable_buttons(True)
        self.rs.baseline_corrected_dict.clear()
        self.rs.baseline_dict.clear()
        for key, baseline, array_corrected in self.baseline_corrected_list:
            self.rs.baseline_corrected_dict[key] = array_corrected
            self.rs.baseline_corrected_not_trimmed_dict[key] = array_corrected
            self.rs.baseline_dict[key] = baseline
        await self.rs.update_plot_item(self.rs.baseline_corrected_dict.items(), 5)
    ...
    async def undo(self) -> None:
        ...
        self.rs.baseline_corrected_dict.clear()
        self.rs.baseline_dict.clear()
        self.rs.baseline_corrected_dict = self.baseline_corrected_dict_before.copy()
        self.rs.baseline_corrected_not_trimmed_dict = self.baseline_corrected_dict_before.copy()
        self.rs.baseline_dict = self.baseline_dict_before.copy()
        await self.rs.update_plot_item(self.baseline_corrected_dict_before.items(), 5)
...
class CommandTrim(QUndoCommand):
    ...
    def __init__(self, rs, cut_list: list[tuple[str, np.ndarray]], description: str) -> None:
        ...
        self.cut_list = cut_list  # 0 - name, 1 - array
        self.setText(description)
        self.baseline_corrected_dict = rs.baseline_corrected_dict
        self.baseline_corrected_dict_before = rs.baseline_corrected_dict.copy()
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
    ...
    async def redo(self) -> None:
        ...
        await self.rs.update_plot_item(self.baseline_corrected_dict.items(), 5)
        create_task(self._stop())
...
class CommandAddDeconvLine(QUndoCommand):
    ...
    def __init__(self, rs, idx: int, line_type: str, description: str) -> None:
        ...
        self._line_params = rs.get_initial_parameters_for_line(line_type)
    ...
    async def redo(self) -> None:
        ...
        self.RS.add_deconv_curve_to_plot(self._line_params, self._idx, self._style, self._line_type)
        create_task(self._stop())
    ...
    async def undo(self) -> None:
        ...
        self.RS.delete_deconv_curve(self._idx)
        self.RS.ui.fit_params_table.model().delete_rows(self._idx)
        create_task(self._stop())

    def update_undo_redo_tooltips(self) -> None:
        self.RS.draw_sum_curve()
        self.RS.draw_residual_curve()
        if self.UndoStack.canUndo():
            self.UndoAction.setToolTip(self.text())
        else:
            self.UndoAction.setToolTip('')
...
class CommandDeleteDeconvLines(QUndoCommand):
    ...
    def __init__(self, rs, description: str) -> None:
        ...
        self._line_params = rs.current_line_parameters(self._selected_row)
        self._line_params['x_axis'] = rs.x_axis_for_line(self._line_params['x0'], self._line_params['dx'])
    ...
    async def redo(self) -> None:
        ...
        self.RS.delete_deconv_curve(self._selected_row)
    ...
    async def undo(self) -> None:
        ...
        self.RS.add_deconv_curve_to_plot(self._line_params, self._selected_row, self._style, self._line_type)
    ...
    async def _stop(self) -> None:
        ...
        self.RS.draw_sum_curve()
        self.RS.draw_residual_curve()
        self.RS.deselect_selected_line()
        self.update_undo_redo_tooltips()
        self.RS.set_modified()
        collect(2)
        await sleep(0)
...
class CommandDeconvLineTypeChanged(QUndoCommand):
    ...
    async def redo(self) -> None:
        ...
        self.RS.redraw_curve(line_type=self.line_type_new, idx=self._idx)
        create_task(self._stop())
    ...
    async def undo(self) -> None:
        ...
        self.RS.redraw_curve(line_type=self.line_type_old, idx=self._idx)
        create_task(self._stop())
    ...
    def update_params_table(self, line_type_new: str, line_type_old: str) -> None:
        ...
        line_params = self.RS.get_initial_parameters_for_line(line_type_new)
...
class CommandUpdateDeconvCurveStyle(QUndoCommand):
    ...
    def __init__(self, rs, idx: int, style: dict, old_style: dict, description: str) -> None:
    ...
    @asyncSlot()
    async def redo(self) -> None:
        self.RS.update_curve_style(self._idx, self._style)
        create_task(self._stop())

    @asyncSlot()
    async def undo(self) -> None:
        self.RS.update_curve_style(self._idx, self._old_style)
        create_task(self._stop())
    ...
    async def _stop(self) -> None:
        # print('CommandUpdateDeconvCurveStyle')
        self.update_undo_redo_tooltips()
        self.RS.set_modified()
        collect(2)
        await sleep(0)
...
class CommandClearAllDeconvLines(QUndoCommand):
    ...
    def __init__(self, rs, description: str) -> None:
        ...
        self.report_result_old = rs.report_result.copy()
        self.sigma3_dict = copy.deepcopy(rs.sigma3)
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
        self.rs = rs
    ...
    async def redo(self) -> None:
        ...
        self.rs.remove_all_lines_from_plot()
        self.rs.report_result.clear()
        self.rs.ui.report_text_edit.setText('')
        self.rs.sigma3.clear()
        self.rs.sigma3_top.setData(x=np.array([0]), y=np.array([0]))
        self.rs.sigma3_bottom.setData(x=np.array([0]), y=np.array([0]))
        self.rs.update_sigma3_curves('')
        create_task(self._stop())
    ...
    async def undo(self) -> None:
        ...
        self.rs.report_result = self.report_result_old
        self.rs.show_current_report_result()
        self.rs.sigma3 = self.sigma3_dict
        filename = '' if self.rs.isTemplate else self.rs.current_spectrum_deconvolution_name
        self.rs.update_sigma3_curves(filename)
        await self.rs.draw_all_curves()
        create_task(self._stop())
...
class CommandAfterFitting(QUndoCommand):
    ...
    def __init__(self, rs, results: list[ModelResult],
        ...
        self.sum_ar = rs.sum_array()
        self.fit_report = ''
        self.report_result_old = rs.report_result[filename] if filename in rs.report_result else ''
        self.sigma3 = self.rs.sigma3[self.filename] if self.filename in self.rs.sigma3 else None
        self.setText(description)
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
    ...
    async def redo(self) -> None:
        ...
        if self.filename != '':
            ...
            self.rs.add_line_params_from_template(self.filename)
        before = datetime.now()
        for fit_result in self.results:
            self.rs.set_parameters_after_fit_for_spectrum(fit_result, self.filename)
        # print(datetime.now() - before)
        x_axis, _ = self.sum_ar
        self.rs.sigma3[self.filename] = x_axis, self.sigma3_conc_up, self.sigma3_conc_bottom
        self.rs.update_sigma3_curves(self.filename)
        self.rs.report_result[self.filename] = self.report_text
    ...
    async def undo(self) -> None:
        ...
        self.rs.ui.fit_params_table.model().concat_df(self.df)
        self.rs.set_rows_visibility()
        x_axis, y_axis = self.sum_ar
        self.rs.sum_curve.setData(x=x_axis, y=y_axis)
        self.rs.report_result[self.filename] = self.report_result_old
        self.rs.show_current_report_result()
        if self.sigma3 is not None:
            self.rs.sigma3[self.filename] = self.sigma3[0], self.sigma3[1], self.sigma3[2]
        else:
            del self.rs.sigma3[self.filename]
            self.rs.fill.setVisible(False)
        self.rs.update_sigma3_curves(self.filename)
        create_task(self._stop())
    ...
    async def _stop(self) -> None:
        ...
        self.rs.redraw_curves_for_filename()
        self.rs.draw_sum_curve()
        self.rs.draw_residual_curve()
        self.rs.show_current_report_result()
        self.rs.update_sigma3_curves()
...
class CommandAfterBatchFitting(QUndoCommand):
    ...
    def __init__(self, rs, results: list[tuple[str, ModelResult]],
        ...
        self.sum_ar = rs.sum_array()
        self.fit_report = ''
        self.report_result_old = copy.deepcopy(rs.report_result)
        self.sigma3 = copy.deepcopy(rs.sigma3)
        self.setText(description)
        self.fit_reports = {}
        self.chisqr_av = {}
        self.redchi_av = {}
    ...
    def create_deconvoluted_dataset_new(self) -> pd.DataFrame:
        if self.rs.is_production_project and self.rs.ui.deconvoluted_dataset_table_view.model().rowCount() != 0:
            filenames = []
        ...
        class_ids = []
        if self.rs.is_production_project:
            for i in filenames:
                class_ids.append(0)
        for filename in filenames:
            if not self.rs.is_production_project:
                class_ids.append(filename_group.loc[filename])
            values = []
    ...
    async def redo(self) -> None:
        ...
        self.rs.add_line_params_from_template_batch(self.keys)
        for key, fit_result in self.results:
            self.rs.set_parameters_after_fit_for_spectrum(fit_result, key)
        x_axis, _ = self.sum_ar
        for key in self.keys:
            self.rs.sigma3[key] = x_axis, self.sigma3_conc_up[key], self.sigma3_conc_bottom[key]
        for key in self.keys:
            self.rs.report_result[key] = self.report_text[key]
        ...
        for i in self.rs.report_result:
            self.rs.report_result[i] += '\n' + '\n' + self.av_text
    ...
    async def undo(self) -> None:
        ...
        self.rs.report_result = copy.deepcopy(self.report_result_old)
        if self.sigma3 is not None:
            self.rs.sigma3 = copy.deepcopy(self.sigma3)
        else:
            self.rs.sigma3.clear()
            self.rs.fill.setVisible(False)
...
class CommandAfterGuess(QUndoCommand):
    ...
    def __init__(self, rs, result: list[ModelResult], line_type: str,
        ...
        self.report_result_old = rs.report_result[self.filename] if self.filename in rs.report_result else ''
        self.sigma3_old = self.rs.sigma3[self.filename] if self.filename in self.rs.sigma3 else None
        self.setText(description)
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
    ...
    async def redo(self) -> None:
        ...
        self.rs.report_result.clear()
        self.rs.report_result[self.filename] = report_text
        ...
        self.rs.sigma3[self.filename] = x_axis, sigma3_conc_up, sigma3_conc_bottom
        self.rs.update_sigma3_curves(self.filename)
        create_task(self._stop())
    ...
    async def undo(self) -> None:
        ...
        if self.sigma3_old is not None:
            self.rs.sigma3[self.filename] = self.sigma3_old[0], self.sigma3_old[1], self.sigma3_old[2]
        else:
            del self.rs.sigma3[self.filename]
            self.rs.fill.setVisible(False)
        self.rs.update_sigma3_curves(self.filename)
    ...
    def process_result(self, fit_result: lmfit.model.ModelResult) -> None:
        ...
        for i, j in enumerate(fit_result.best_values.items()):
            ...
            if i % k == k - 1:
                self.rs.add_deconv_curve_to_plot(line_params, idx, rnd_style, self.line_type)
...
class CommandAfterFittingStat(QUndoCommand):
    ...
    def __init__(self, rs, result: dict, cl_type: str, description: str) -> None:
        ...
        if cl_type not in rs.latest_stat_result:
            self.stat_result_old = None
        else:
            self.stat_result_old = copy.deepcopy(rs.latest_stat_result[cl_type])
        self.UndoAction = rs.action_undo
        self.RedoAction = rs.action_redo
        self.UndoStack = rs.undoStack
        self.rs = rs
    ...
    def create_stat_result(self) -> dict:
        ...
        elif self.cl_type == 'Nearest Neighbors':
            stat_result_new = self.create_stat_result_nearest()
        elif self.cl_type == 'GPC':
            stat_result_new = self.create_stat_result_gpc()
        elif self.cl_type == 'Decision Tree':
            stat_result_new = self.create_stat_result_dt()
        elif self.cl_type == 'Naive Bayes':
            stat_result_new = self.create_stat_result_nb()
        elif self.cl_type == 'Random Forest':
            stat_result_new = self.create_stat_result_rf()
        elif self.cl_type == 'AdaBoost':
            stat_result_new = self.create_stat_result_ab()
        elif self.cl_type == 'MLP':
            stat_result_new = self.create_stat_result_mlp()
    ...
    def create_stat_result_lda(self) -> dict:
        accuracy_score_train = self.result['accuracy_score_train']
        feature_names = self.result['feature_names']
        model = self.result['model']
        model_2d = self.result['model_2d']
        target_names = self.result['target_names']
        features_in_2d = self.result['features_in_2d']
        y_score = self.result['y_score']
        y_test_bin = self.result['y_test_bin']
        y_train = self.result['y_train']
        y_test = self.result['y_test']
        y_pred = self.result['y_pred']
        cv_scores = self.result['cv_scores']
        if 'y_pred_2d' in self.result:
            y_pred_2d = self.result['y_pred_2d']
        else:
            y_pred_2d = None
        y_pred_test = self.result['y_pred_test']
        X = self.result['X']
        x_train = self.result['x_train']
        x_test = self.result['x_test']
        y_score_dec_func = self.result['y_score_dec_func']
        explained_variance_ratio = self.result['explained_variance_ratio'] if 'explained_variance_ratio' in self.result\
            else None
        label_binarizer = LabelBinarizer().fit(y_train)
        y_onehot_test = label_binarizer.transform(y_test)
        binary = False
        if features_in_2d.shape[1] == 1:
            binary = True
        else:
            classifier = OneVsRestClassifier(make_pipeline(StandardScaler(), model))
            classifier.fit(x_train, y_train)
            y_score_dec_func = classifier.decision_function(x_test)
        metrics_result = model_metrics(y_test, y_pred_test, binary, target_names)
        metrics_result['accuracy_score_train'] = accuracy_score_train
        y_train_plus_test = np.concatenate((y_train, y_test))
        if self.cl_type == 'LDA':
            explainer = shap.Explainer(model.best_estimator_, X)
        ...
        return {'model': model, 'features_in_2d': features_in_2d, 'y_train_plus_test': y_train_plus_test,
                'y_pred': y_pred, 'x_train': x_train, 'feature_names': feature_names,
                'target_names': target_names, 'features_in_2d': features_in_2d, 'model_2d': model_2d,
                'y_test': y_test, 'y_pred_test': y_pred_test, 'metrics_result': metrics_result,
                'y_score': y_score, 'y_onehot_test': y_onehot_test, 'x_test': x_test,
                'y_score_dec_func': y_score_dec_func, 'y_test_bin': y_test_bin,
                'shap_values': shap_values, 'explained_variance_ratio': explained_variance_ratio,
                'y_pred_2d': y_pred_2d, 'cv_scores': cv_scores, 'explainer': explainer,
                'shap_values_legacy': shap_values_legacy}

    def create_stat_result_lr(self) -> dict:
        y_test = self.result['y_test']
        lr_result = copy.deepcopy(self.result)
        lr_result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
        ...
        lr_result['metrics_result'] = metrics_result
        label_binarizer = LabelBinarizer().fit(self.result['y_train'])
        lr_result['y_onehot_test'] = label_binarizer.transform(y_test)
        explainer = shap.Explainer(self.result['model'].best_estimator_, self.result['X'])
        lr_result['shap_values'] = explainer(self.result['X'])
        lr_result['explainer'] = explainer
        lr_result['shap_values_legacy'] = explainer.shap_values(self.result['X'])
        return lr_result

    def create_stat_result_svc(self) -> dict:
        y_test = self.result['y_test']
        svc_result = copy.deepcopy(self.result)
        svc_result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
        binary = True if len(self.result['model'].classes_) == 2 else False
        metrics_result = model_metrics(y_test, self.result['y_pred_test'], binary, self.result['target_names'])
        metrics_result['accuracy_score_train'] = self.result['accuracy_score_train']
        svc_result['metrics_result'] = metrics_result
        label_binarizer = LabelBinarizer().fit(self.result['y_train'])
        svc_result['y_onehot_test'] = label_binarizer.transform(y_test)
        kernel_explainer = shap.KernelExplainer(self.result['model'].best_estimator_.predict_proba, self.result['X'])
        explainer = shap.Explainer(self.result['model'].best_estimator_, self.result['X'])
        svc_result['shap_values'] = explainer(self.result['X'])
        svc_result['explainer'] = kernel_explainer
        svc_result['shap_values_legacy'] = kernel_explainer.shap_values(self.result['X'])
        return svc_result

    def create_stat_result_nearest(self) -> dict:
        y_test = self.result['y_test']
        x_train = self.result['x_train']
        ...
        func = lambda x: self.result['model'].best_estimator_.predict_proba(x)[:, 1]
        med = x_train.median().values.reshape((1, x_train.shape[1]))
        kernel_explainer = shap.KernelExplainer(func, med)
        explainer = shap.Explainer(func, med)
    ...
    def create_stat_result_gpc(self) -> dict:
        y_test = self.result['y_test']
        x_train = self.result['x_train']
        result = copy.deepcopy(self.result)
        result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
        ...
        label_binarizer = LabelBinarizer().fit(self.result['y_train'])
        result['y_onehot_test'] = label_binarizer.transform(y_test)
        func = lambda x: self.result['model'].predict_proba(x)[:, 1]
        med = x_train.median().values.reshape((1, x_train.shape[1]))
        kernel_explainer = shap.KernelExplainer(func, med)
        explainer = shap.Explainer(func, med)
        result['shap_values'] = explainer(self.result['X'])
    ...
    def create_stat_result_dt(self) -> dict:
        y_test = self.result['y_test']
        x_train = self.result['x_train']
        result = copy.deepcopy(self.result)
        result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
    ...
    def create_stat_result_nb(self) -> dict:
        y_test = self.result['y_test']
        x_train = self.result['x_train']
        result = copy.deepcopy(self.result)
        result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
    ...
    def create_stat_result_ab(self) -> dict:
        y_test = self.result['y_test']
        x_train = self.result['x_train']
        result = copy.deepcopy(self.result)
        result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
    ...
    def create_stat_result_mlp(self) -> dict:
        y_test = self.result['y_test']
        x_train = self.result['x_train']
        result = copy.deepcopy(self.result)
        result['y_train_plus_test'] = np.concatenate((self.result['y_train'], y_test))
    ...
    async def redo(self) -> None:
        ...
        if self.stat_result_new is not None:
            self.rs.latest_stat_result[self.cl_type] = self.stat_result_new
        create_task(self._stop())

    @asyncSlot()
    async def undo(self) -> None:
        info('undo CommandAfterFittingStat')
        self.rs.latest_stat_result[self.cl_type] = self.stat_result_old
        if self.stat_result_old is None:
            self.rs.clear_selected_step(self.cl_type)
        create_task(self._stop())
    ...
    async def _stop(self) -> None:
        if self.cl_type == 'LDA' and self.rs.latest_stat_result['LDA'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_lda_plots)
        elif self.cl_type == 'QDA' and self.rs.latest_stat_result['QDA'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_qda_plots)
        elif self.cl_type == 'Logistic regression' and self.rs.latest_stat_result['Logistic regression'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_lr_plots)
        elif self.cl_type == 'NuSVC' and self.rs.latest_stat_result['NuSVC'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_svc_plots)
        elif self.cl_type == 'Nearest Neighbors' and self.rs.latest_stat_result['Nearest Neighbors'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_nearest_plots)
        elif self.cl_type == 'GPC' and self.rs.latest_stat_result['GPC'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_gpc_plots)
        elif self.cl_type == 'Decision Tree' and self.rs.latest_stat_result['Decision Tree'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_dt_plots)
        elif self.cl_type == 'Naive Bayes' and self.rs.latest_stat_result['Naive Bayes'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_nb_plots)
        elif self.cl_type == 'Random Forest' and self.rs.latest_stat_result['Random Forest'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_rf_plots)
        elif self.cl_type == 'AdaBoost' and self.rs.latest_stat_result['AdaBoost'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_ab_plots)
        elif self.cl_type == 'MLP' and self.rs.latest_stat_result['MLP'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_mlp_plots)
        elif self.cl_type == 'PCA' and self.rs.latest_stat_result['PCA'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_pca_plots)
        elif self.cl_type == 'PLS-DA' and self.rs.latest_stat_result['PLS-DA'] is not None:
            self.rs.loop.run_in_executor(None, self.rs.update_plsda_plots)
        self.rs.update_force_single_plots(self.cl_type)
        self.rs.update_force_full_plots(self.cl_type)
        self.rs.update_stat_report_text()
        info('stop CommandAfterFittingStat')
        self.update_undo_redo_tooltips()
        # print('stop CommandAfterFittingStat')
        self.rs.set_modified()
        ...
        seconds = round((datetime.now() - time_end).total_seconds())
        self.rs.beforeTime = None
        self.rs.ui.statusBar.showMessage('Model fitting completed for ' + str(seconds) + ' sec.', 55000)
        collect(2)
        await sleep(0)
