...
from modules.functions_fitting import fitting_model, fit_model
from modules.functions_for_arrays import find_nearest, nearest_idx
...
def clustering_lines_intervals(data_by_intervals: dict, hwhm: float) -> list[ndarray]:
    """
...
def clusters_means(x0_merged: list[list], std: bool = False) -> np.ndarray:
    ...
    array([585.995, 851.78, 814.205, 875.585, 611.82, 564.575, 687.80666667, 759.66, 788.97, 835.62, 721.89, 645.24,
       776.64, 658.56])
    """
    x0_means = np.zeros(len(x0_merged))
    x0_std = np.zeros_like(x0_means)
    for j, x0s in enumerate(x0_merged):
        x0_means[j] = np.mean(x0s)
        if std:
            x0_std[j] = np.std(x0s)

    return np.stack((x0_means, x0_std)) if std else x0_means
