...
class PredictLogic:
    ...
    async def do_predict_production(self) -> None:
        ...
        filenames = list(main_window.context.preprocessing.stages.input_data.data.keys())
        interpolated = await main_window.preprocessing.get_interpolated(filenames,
                                                                        self.interp_ref_array)
        ...
        if main_window.ui.dataset_type_cb.currentText() == 'Smoothed':
            ...
            main_window.context.set_modified()
        ...
        df = main_window.ui.deconvoluted_dataset_table_view.model().dataframe().reset_index(
            drop=True)
    ...
    async def do_predict(self) -> None:
        ...
        with Manager() as manager:
            ...
            with executor:
                if self.is_production_project:
                    main_window.futures = [
                        main_window.loop.run_in_executor(executor, clf_predict, X,
    ...
    def update_predict_table(self, predict_data, classificators_names: list[str],
                             filenames: list[str]) \
            -> None:
        """
