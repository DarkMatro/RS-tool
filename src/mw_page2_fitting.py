...
from pyqtgraph import PlotCurveItem, ROI, mkBrush, PlotDataItem, ErrorBarItem, mkPen
from qtpy.QtCore import QTimer
from qtpy.QtGui import QMouseEvent, QColor
from scipy.signal import argrelmin

from qfluentwidgets import MessageBox
from src.stages.fitting.functions.fitting import (curve_idx_from_par_name, fit_model_batch,
                                                  split_by_borders, fitting_model, fit_model)
from src.data.work_with_arrays import nearest_idx, find_nearest
from src.mutual_functions.static_functions import (packed_current_line_parameters,
                                                   models_params_splitted_batch,
                                                   models_params_splitted, eval_uncert,
                                                   get_curve_for_deconvolution,
                                                   curve_pen_brush_by_style, set_roi_size,
                                                   set_roi_size_pos)
from src.stages.preprocessing.functions.averaging import get_average_spectrum
from src.stages.fitting.functions.guess_raman_lines import (clustering_lines_intervals,
                                                            intervals_by_borders,
                                                            create_intervals_data,
                                                            params_func_legend, legend_from_float,
                                                            update_fit_parameters, guess_peaks)

from src.data.default_values import peak_shape_params_limits, fitting_methods, peak_shapes_params
from src.stages.preprocessing.functions import cut_full_spectrum, cut_axis
from src.undo_redo import CommandAfterBatchFitting, CommandAfterFitting, CommandAfterGuess, \
    CommandDeconvLineDragged, CommandDeconvLineTypeChanged, CommandEndIntervalChanged, \
    CommandStartIntervalChanged
...
class FittingLogic:
    ...
    def __init__(self, parent):
        ...
        self.sigma3 = dict()
        self.params_stderr = defaultdict(
            dict)  # key - filename; value - dict with key - curve_idx and value - dict
        #  with key - param_name; value - stderr. Example  {'filename1': {0: {'a': 0.1, 'x0': 1.1, 'dx': .5},...}, ...}
    ...
    async def do_batch_fit(self) -> None:
        ...
        stage = self.parent.drag_widget.get_latest_active_stage()
        ...
        if main_window.predict_logic.is_production_project \
            ...
            filenames_in_dataset = list(
                main_window.ui.deconvoluted_dataset_table_view.model().column_data(1).values)
            ...
            for key in filenames:
                try:
                    baseline_corrected_spectra[key] = [
                        data[key]]
                except KeyError:
                    pass
        ...
        list_params_full = self._fitting_params_batch(idx_type_param_count_legend_func,
                                                      baseline_corrected_spectra)
        ...
        main_window.open_progress_dialog("Batch Fitting...", "Cancel",
                                         maximum=intervals if intervals > 1 else 0)
        with Manager() as manager:
            main_window.break_event = manager.Event()
            with main_window.ex as ex:
                main_window.futures = [
                    main_window.loop.run_in_executor(ex, fit_model_batch, model, y, params,
                                                     x, method, key)
        ...
        with Manager() as manager:
            ...
            with executor:
                main_window.futures = [
                    main_window.loop.run_in_executor(main_window.ex, eval_uncert, i)
                    for i in result]
                for future in main_window.futures:
                    future.add_done_callback(main_window.progress_indicator)
                dely = await gather(*main_window.futures)
        main_window.close_progress_bar()
        main_window.open_progress_bar(max_value=0)
        main_window.open_progress_dialog("updating data...", "Cancel", maximum=0)
        command = CommandAfterBatchFitting(main_window, result, idx_type_param_count_legend_func,
                                           dely,
    ...
    async def do_fit(self) -> None:
        ...
        main_window.open_progress_bar(max_value=intervals if intervals > 1 else 0)
        main_window.open_progress_dialog("Fitting...", "Cancel",
                                         maximum=intervals if intervals > 1 else 0)
        idx_type_param_count_legend_func = self.prepare_data_fitting()
        params = self.fitting_params(idx_type_param_count_legend_func, np.max(arr[:, 1]),
                                     arr[:, 0][0],
                                     arr[:, 0][-1])
        x_y_models_params, _ = models_params_splitted(splitted_arrays, params,
                                                      idx_type_param_count_legend_func)
        method_full_name = main_window.ui.fit_opt_method_comboBox.currentText()
        method = self.fitting_methods[method_full_name]
        executor = ProcessPoolExecutor()
        ...
        if main_window.cancelled_by_user():
            return
        command = CommandAfterFitting(main_window, result, idx_type_param_count_legend_func,
                                      spec_name,
    ...
    async def do_auto_guess(self, line_type: str) -> None:
        ...
        mw.ex = ProcessPoolExecutor()
        with Manager() as manager:
            mw.break_event = manager.Event()
            with mw.ex:
                mw.futures = [mw.loop.run_in_executor(mw.ex, guess_peaks, arr,
                ...
                latest_guess_result = await gather(*mw.futures)
        ...
        if mw.ui.guess_method_cb.currentText() != 'Average':
            ...
            mw.open_progress_dialog("Fitting...", "Cancel",
                                    maximum=intervals if intervals > 1 else 0)
            mw.ex = ProcessPoolExecutor()
            with Manager() as manager:
                mw.break_event = manager.Event()
                with mw.ex:
                    mw.futures = [
                        mw.loop.run_in_executor(mw.ex, fit_model, i[2], i[1], i[3], i[0],
                                                parameters_to_guess['method'])
                        for i in x_y_models_params]
                    for future in mw.futures:
                        future.add_done_callback(mw.progress_indicator)
                    result = await gather(*mw.futures)
        else:
    ...
    async def switch_to_template(self, _: str = 'Average') -> None:
        self.update_single_deconvolution_plot(self.parent.ui.template_combo_box.currentText(), True,
                                              True)
    ...
    def _split_array_for_fitting(self, arrays: dict[str, list[np.ndarray]]) -> dict[
        str, list[np.ndarray]]:
        """
        ...
        if self.parent.ui.interval_checkBox.isChecked():
            for key, arr in arrays.items():
                split_arrays[key] = [
                    cut_full_spectrum(arr[0], self.parent.ui.interval_start_dsb.value(),
    ...
    def _fitting_params_batch(self, idx_type_param_count_legend_func: list[
        ...
        try:
            ...
            x_axis = self.get_x_axis()
        params_mutual = self.fitting_params(idx_type_param_count_legend_func, 1000., x_axis[0],
                                            x_axis[-1])
        list_params_full = {}
    ...
    def fitting_params(self, list_idx_type: list[tuple[int, str, int, str, callable]],
        ...
        if self.current_spectrum_deconvolution_name == '':
            ...
            row_data = self.parent.ui.input_table.model().row_data_by_index(
                self.current_spectrum_deconvolution_name)
        ...
        for idx, line_type, param_count, legend, _ in list_idx_type:
            ...
            for param_name in param_names:
                ...
                v = min_v + 1e-3 if v <= min_v else v
                v = max_v - 1e-3 if v >= max_v else v
                if min_v == max_v:
                    max_v += 1e-3
                if param_name == 'a':
                    max_v = bound_max_a
    ...
    def current_line_parameters(self, index: int, filename: str | None = None) -> dict | None:
        ...
        line_type = self.parent.ui.deconv_lines_table.model().cell_data_by_idx_col_name(index,
                                                                                        'Type')
        if df_params.empty:
            return None
    ...
    def array_of_current_filename_in_deconvolution(self) -> np.ndarray | None:
        ...
        arr = None
        averaged_dict = self.parent.context.preprocessing.stages.av_data.data
        stage = self.parent.drag_widget.get_latest_active_stage()
        assert stage is not None, 'Cant find latest active stage.'
        data = stage.data
        if self.is_template:
            ...
            elif averaged_dict and averaged_dict != {}:
                arr = averaged_dict[
                    int(self.parent.ui.template_combo_box.currentText().split('.')[0])]
        elif current_spectrum_name in data:
    ...
    def set_rows_visibility(self) -> None:
        ...
        idx = self.parent.ui.deconv_lines_table.model().row_data(row).name
        row_id_to_show = self.parent.ui.fit_params_table.model().row_number_for_filtering(
            (filename, idx))
    ...
    def update_template_combo_box(self) -> None:
        ...
        data = self.parent.context.preprocessing.stages.av_data.data
        if (not data
                or self.parent.context.group_table.table_widget.model().rowCount() == 0):
            return
        for i in data:
            group_name = self.parent.context.group_table.table_widget.model().row_data(i - 1)['Group name']
            self.parent.ui.template_combo_box.addItem(str(i) + '. ' + group_name)
        self.parent.ui.template_combo_box.currentTextChanged.connect(
            lambda: self.switch_to_template())

    def update_deconv_intervals_limits(self) -> None:
        mw = self.parent
        data = mw.context.preprocessing.stages.av_data.data
    ...
    def create_deconvoluted_dataset_new(self) -> Any | None:
        ...
        x_axis = self.array_of_current_filename_in_deconvolution()[:, 0]
        ...
        line_legends_target_param = []
        line_legends_x0 = []
        use_area = environ['target_peak_parameter'] == 'Area'
        target_char = 's' if use_area else 'a'
        include_x0 = self.parent.ui.include_x0_checkBox.isChecked()
        ...
        if include_x0:
            ...
            line_legends = line_legends_target_param
        params = self.parent.ui.fit_params_table.model().column_data(1)
        idx_line_type = self.parent.ui.deconv_lines_table.model().column_data(1)
        ...
        # exact_values_a = self.parent.ui.fit_params_table.model().get_df_by_filename('').iloc[:, 1: 2].query(
        #     "param_name == 'a'").values.T
        # exact_values_x0 = self.parent.ui.fit_params_table.model().get_df_by_filename('').iloc[:, 1: 2].query(
        #     "param_name == 'x0'").values.T
        # exact_values_dx = self.parent.ui.fit_params_table.model().get_df_by_filename('').iloc[:, 1: 2].query(
        #     "param_name == 'dx'").values.T
        # stderr_a = np.array([d['a'] for d in list(self.params_stderr[''].values())])
        # stderr_x0 = np.array([d['x0'] for d in list(self.params_stderr[''].values())])
        # stderr_dx = np.array([d['dx'] for d in list(self.params_stderr[''].values())])
        # rel_stderr_a = stderr_a / exact_values_a
        # rel_stderr_a = rel_stderr_a.T
        # rel_stderr_x0 = stderr_x0 / exact_values_x0
        # rel_stderr_x0 = rel_stderr_x0.T
        # rel_stderr_dx = stderr_dx / exact_values_dx
        # rel_stderr_dx = rel_stderr_dx.T
        for filename in filenames:
            ...
            #         # err_a = rel_stderr_a[i]
            #         # err_x0 = rel_stderr_x0[i]
            #         # err_dx = rel_stderr_dx[i]
            #         line_type = idx_line_type[i]
            #         p = params.loc[(filename, i)]
            #         # p_a = p['a']
            #         # p['a'] = np.random.uniform(p_a - p_a * err_a, p_a + p_a * err_a)[0]
            #         # p_x0 = p['x0']
            #         # p['x0'] = np.random.uniform(p_x0 - p_x0 * err_x0, p_x0 + p_x0 * err_x0)[0]
            #         # p_dx = p['dx']
            #         # p['dx'] = np.random.uniform(p_dx - p_dx * err_dx, p_dx + p_dx * err_dx)[0]
            #         y, x, _ = self.curve_y_x_idx(line_type, p, x_axis, i)
            #         s_tr = np.trapz(y, x)
            #         values_target_param.append(s_tr)
            # else:
            for i in line_indexes:
                # err_a = rel_stderr_a[i]
                v = params.loc[(filename, i, 'a')]
                # v = np.random.uniform(v - v * err_a, v + v * err_a)[0]
                values_target_param.append(v)
            values_x0 = []
            if include_x0:
                for i in line_indexes:
                    v = params.loc[(filename, i, 'x0')]
                    values_x0.append(v)
                values = np.concatenate((values_target_param, values_x0))
            else:
                values = values_target_param
    ...
    def _parameters_to_guess(self, line_type: str) -> dict:
        ...
        return {'func': func, 'param_names': param_names, 'init_model_params': init_model_params,
                'min_fwhm': min_fwhm,
                'method': method, 'params_limits': params_limits, 'noise_level': noise_level,
                'max_dx': max_dx,
    ...
    def _param_names(self, line_type: str) -> list[str]:
        ...
        """
        param_names = ['a', 'x0', 'dx']
        if line_type in self.peak_shapes_params and 'add_params' in self.peak_shapes_params[
            line_type]:
    ...
    def _initial_template(self, visible_lines: DataFrame, func,
        ...
        for i in visible_lines.index:
            ...
            if 'dx_left' in static_parameters[0]:
                dx_left_series = self.parent.ui.fit_params_table.model().get_df_by_multiindex(
                    ('', i, 'dx_left'))
    ...
    def _arrays_for_peak_guess(self) -> list[ndarray]:
        ...
        if guess_method == 'Average groups':
            ...
            for key, arr in self.parent.context.preprocessing.stages.av_data.data.items():
                arrays[key] = [arr]
    ...
    async def _analyze_guess_results(self, result: list[ModelResult], param_names: list[str],
        ...
        x_y_models_params = self._models_params_after_guess(self.all_ranges_clustered_x0_sd,
                                                            param_names, line_type)
    ...
    def _create_intervals_data(self, result: list[ModelResult], peak_n_params: int) \
        ...
        if self.parent.ui.interval_checkBox.isChecked():
            intervals = [(self.parent.ui.interval_start_dsb.value(),
                          self.parent.ui.interval_end_dsb.value())]
    ...
    def _models_params_after_guess(self, all_ranges_clustered_x0_sd: list[np.ndarray],
                                   param_names: list[str],
                                   line_type: str) -> list[
        tuple[np.ndarray, np.ndarray, Model, Parameters]]:
        """
        ...
        """
        sliced_average_array_by_ranges = \
        self._split_array_for_fitting({'Average': [self.averaged_spectrum]})['Average']
    ...
    def create_roi_curve_add_to_plot(self, full_amp_line: np.ndarray | None,
        ...
        roi.sigRegionChanged.connect(
            lambda checked=None, index=idx: self.curve_roi_pos_changed(index, roi, curve))
    ...
    def update_curve_fill_realtime(self):
        ...
        if self.parent.ui.deconv_lines_table.model().rowCount() == 0:
            return
        curve_style = self.parent.ui.deconv_lines_table.model().cell_data_by_idx_col_name(idx,
                                                                                          'Style')
    ...
    def deselect_selected_line(self) -> None:
        if self.timer_fill is not None:
            ...
            if self.updating_fill_curve_idx is not None \
                    and self.updating_fill_curve_idx in list(
                self.parent.ui.deconv_lines_table.model().indexes):
                curve_style = self.parent.ui.deconv_lines_table.model().cell_data_by_idx_col_name(
                    self.updating_fill_curve_idx,
                    'Style')
                self.update_curve_style(self.updating_fill_curve_idx, curve_style)
    ...
    def curve_roi_pos_change_finished(self, index: int, roi: ROI) -> None:
        ...
        if (a, x0, dx) != self.dragged_line_parameters and (
        a, x0, dx) != self.prev_dragged_line_parameters and \
                (a, x0, dx) != (roi_a, roi_x0, roi_dx) and self.CommandDeconvLineDraggedAllowed:
            command = CommandDeconvLineDragged(self.parent, (a, x0, dx),
                                               self.dragged_line_parameters,
    ...
    def redraw_curve(self, params: dict | None = None, curve: PlotCurveItem = None,
        ...
        if line_type is None and idx is not None:
            line_type = self.parent.ui.deconv_lines_table.model().cell_data_by_idx_col_name(idx,
                                                                                            'Type')
    ...
    def draw_sum_curve(self) -> None:
        ...
        assert stage is not None, 'Cant find latest active stage.'
        data = stage.data
        if not data or self.sum_curve is None:
            return
    ...
    def draw_residual_curve(self) -> None:
        ...
        assert stage is not None, 'Cant find latest active stage.'
        data = stage.data
        if not data or self.residual_curve is None:
            return
    ...
    def update_single_deconvolution_plot(self, current_spectrum_name: str,
        ...
        if is_template and is_averaged_or_group:
            self.current_spectrum_deconvolution_name = ''
            averaged_dict = mw.context.preprocessing.stages.av_data.data
            if current_spectrum_name == 'Average':
                arrays_list = [data[x] for x in data]
        ...
        new_title = "<span style=\"font-family: AbletonSans; color:" + environ[
        ...
        self.data_curve = mw.get_curve_plot_data_item(arr, color=self.data_style['color'],
                                                      name='data')
        x_axis, y_axis = self.sum_array()
        self.sum_curve = mw.get_curve_plot_data_item(np.vstack((x_axis, y_axis)).T,
                                                     color=self.sum_style['color'],
                                                     name='sum')
        x_res, y_res = self.residual_array()
        self.residual_curve = mw.get_curve_plot_data_item(np.vstack((x_res, y_res)).T,
                                                          color=self.residual_style['color'],
                                                          name='sum')
        ...
        mw.deconvolution_plotItem.addItem(self.residual_curve,
    ...
    def redraw_curves_for_filename(self) -> None:
        ...
        filename_lines_indexes = self.parent.ui.fit_params_table.model().get_lines_indexes_by_filename(
            filename)
        ...
        if len(line_indexes) != len(filename_lines_indexes):
            MessageBox("Template error",
                       "Состав линий этого спектра отличается от шаблона. Некоторые линии не"
        ...
        if items is None or not items:
            return
        create_errorbars = filename in self.params_stderr and self.params_stderr[filename]
        x, y, h, w = [], [], [], []
        # Remove old ErrorBarItem.
        for i in self.parent.deconvolution_plotItem.items:
        ...
        for i in line_indexes:
            ...
            a = params[i]['a']
            dx = params[i]['dx']
            set_roi_size_pos((a, x0, dx), items[i][1], False)
            self.redraw_curve(params[i], items[i][0], line_types.loc[i], i)
            # Errors bars using stderr.
            if not create_errorbars:
                continue
            x.append(x0)
            x.append(x0 + dx)
            y.append(a)
            y.append(a / 2)
            stderr = self.params_stderr[filename][i]
            h.append(stderr['a'])
            h.append(0.)
            w.append(stderr['x0'])
            w.append(stderr['dx'])
        if not create_errorbars:
            return
        eb = ErrorBarItem(x=np.array(x), y=np.array(y), height=2 * np.array(h),
                          width=2 * np.array(w), x_beam=1, y_beam=.005,
                          pen=mkPen(color='black', width=2))
        self.parent.deconvolution_plotItem.addItem(eb)
    ...
    async def copy_spectrum_lines_parameters_from_template(self) -> None:
        ...
        with Manager() as manager:
            ...
            with executor:
                self.parent.futures = [
                    self.parent.loop.run_in_executor(executor,
                                                     self.copy_line_parameters_from_template, i,
                                                     selected_filename)
                    for i in line_indexes]
                await gather(*self.parent.futures)
    ...
    def interval_start_dsb_change_event(self, new_value: float) -> None:
        ...
        """
        mw = self.parent
        mw.context.set_modified()
        corrected_value = None
        # correct value - take nearest from x_axis
        averaged_dict = mw.context.preprocessing.stages.av_data.data
        if averaged_dict != {} and averaged_dict:
            x_axis = next(iter(averaged_dict.values()))[:, 0]
        ...
        if mw.CommandStartIntervalChanged_allowed:
            ...
            mw.undoStack.push(command)
        mw.linearRegionDeconv.setRegion(
    ...
    def deriv(self):
        ...
        arr = self.array_of_current_filename_in_deconvolution()
        splitted_arrays = self._split_array_for_fitting({'': [arr]})[''][0]
        print(splitted_arrays)
        cm = splitted_arrays[:, 0]
        raman = splitted_arrays[:, 1]
        d1 = np.diff(raman, append=0)
        d2 = np.diff(d1, append=0)
        # d2[d2 > 0] = 0
        d3 = np.diff(d2, append=0)
        d4 = np.diff(d3, append=0)
        d4[d4 < 0] = 0
        amp = 3 * (np.abs(d2) ** 2) / d4
        sig = np.sqrt(3 * (d2 * -1) / d4)
        d1sign = np.sign(d1)
        d1signchange = ((np.roll(d1sign, 1) - d1sign) != 0).astype(int)
        d3sign = np.sign(d3)
        d3signchange = ((np.roll(d3sign, 1) - d3sign) != 0).astype(int)
        z = (d2 < 0) * (d4 > 0)
        fig, ax = plt.subplots()
        ax.plot(cm, raman * 0.1, label='Исходный спектр * 0.1', color='black')
        ax.plot(cm, d1, label='d1', color='orange', linestyle='dashed')
        ax.plot(cm, d2, label='d2')
        ax.plot(cm, d3, label='d3', linestyle='dashed')
        ax.plot(cm, d4, label='d4')
        ax.set_xlabel('Raman shift, cm\N{superscript minus}\N{superscript one}')
        ax.set_ylabel('Intensity, rel. un.')
        # ax.plot(cm, amp, label='amp')
        # ax.plot(cm, 2.355* sig, label='sig')
        ax.grid()
        ax.legend()
        plt.show()
