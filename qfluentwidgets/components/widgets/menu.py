...
from ...common.config import is_dark_theme
...
class ShortcutMenuItemDelegate(MenuItemDelegate):
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        if not option.state & QStyle.State_Enabled:
            painter.setOpacity(0.5 if is_dark_theme() else 0.6)

        font = getFont(12)
        painter.setFont(font)
        painter.setPen(QColor(255, 255, 255, 200) if is_dark_theme() else QColor(0, 0, 0, 153))
...
class RadioIndicatorMenuItemDelegate(CheckableMenuItemDelegate):
    ...
    def _drawIndicator(self, painter, option, index):
        ...
        if not option.state & QStyle.State_MouseOver:
            painter.setOpacity(0.75 if is_dark_theme() else 0.65)

        painter.setPen(Qt.NoPen)
        painter.setBrush(Qt.white if is_dark_theme() else Qt.black)
