...
class Ui_MessageBox:
    ...
    def _setUpUi(self, title, content, parent):
        ...
        self.yesButton = PrimaryPushButton(self.tr('Yes' if 'Yes' in self.buttons else 'Ok'), self.buttonGroup)

        if 'Cancel' in self.buttons:
            self.cancelButton = QPushButton(self.tr('Cancel'), self.buttonGroup)
        if 'No' in self.buttons:
            self.noButton = QPushButton(self.tr('No'), self.buttonGroup)
    ...
    def __initWidget(self):
        ...
        if 'No' in self.buttons:
            self.noButton.setAttribute(Qt.WA_LayoutUsesWidgetRect)
    ...
    def __initLayout(self):
        ...
        if 'No' in self.buttons:
            self.buttonLayout.addWidget(self.noButton, 1, Qt.AlignVCenter)
        if 'Cancel' in self.buttons:
...
class MessageBox(MaskDialogBase, Ui_MessageBox):
    ...
    def __init__(self, title: str, content: str, parent=None, buttons=None):
        super().__init__(parent=parent, buttons=buttons)
